...
const ConceptMap = () => {
  // 定義七大核心能力 (SIRIPPP)
  const competencies = [
    { subject: 'S: Systems (制度體系)', full: 'Systems-based Practice', icon: <GitCommit size={16} /> },
    { subject: 'I: Interpersonal (溝通技巧)', full: 'Interpersonal & Communication', icon: <Users size={16} /> },
    { subject: 'R: Reflection (反思學習)', full: 'Practice-based Learning', icon: <Brain size={16} /> },
    { subject: 'I: Info/Knowledge (醫學知識)', full: 'Medical Knowledge', icon: <BookOpen size={16} /> },
    { subject: 'P: Professionalism (專業素養)', full: 'Professionalism', icon: <Shield size={16} /> },
    { subject: 'P: Patient Care (病人照護)', full: 'Patient Care', icon: <HeartPulse size={16} /> },
    { subject: 'P: Partners (跨域團隊)', full: 'Inter-professional Practice', icon: <Activity size={16} /> },
  ];

  // 根據台灣呼吸治療全聯會發布之公版 11 項 EPA 更新
  const epaScenarios = [
    {
      id: 'epa1',
  ...
  // 預設選取一個較具代表性的 EPA (例如 EPA 6 呼吸器初始設定)
  const [selectedEPAs, setSelectedEPAs] = useState([epaScenarios[5]]);

  // 合併多個 EPA 的資料成為 Recharts 可以讀的格式
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-white p-4 rounded-xl shadow-lg border border-slate-200 flex flex-col items-center min-h-[500px]">
          ...
          <div className="w-full h-[450px] md:h-[600px] relative">
            ...
            {/* Metaphor Explanation Overlay */}
            <div className="absolute bottom-4 right-4 bg-white/95 p-3 rounded-lg border border-slate-200 text-xs shadow-md max-w-[200px] z-10">
        ...
        <div className="lg:col-span-4 flex flex-col gap-4">
          {/* EPA Multi-Selection Control */}
          <div className="bg-white p-5 rounded-xl shadow-md border border-slate-200 flex-1 overflow-y-auto max-h-[600px]">
            <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2 sticky top-0 bg-white pb-2 border-b border-slate-100 z-10">
              <GitCommit className="text-purple-600" />
              EPA 任務清單 (可複選)
            </h3>
            <div className="space-y-2">
              {epaScenarios.map((epa) => {
                const isSelected = selectedEPAs.some((item) => item.id === epa.id);
                return (
                  <button
                    key={epa.id}
                    onClick={() => toggleEPA(epa)}
                    className={`w-full text-left p-3 rounded-lg border transition-all duration-200 flex items-center gap-3 group ${
                      isSelected
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div
                        ...
                        style={{ color: isSelected ? epa.color : '#475569' }}
                      >
                        {epa.name.split(':')[0]}
                      </div>
                    </div>
                    ...
                    {isSelected && (
                      <div className="w-2 h-2 rounded-full" style={{ backgroundColor: epa.color }} />
                    )}
                  </button>
                );
              })}
